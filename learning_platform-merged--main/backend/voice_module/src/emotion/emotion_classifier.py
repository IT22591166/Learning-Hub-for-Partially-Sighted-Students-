...
class EmotionClassifier:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        # Check for model path from environment variable or parameter
        import os
        model_path = model_path or os.getenv('EMOTION_MODEL_PATH') or 'models/emotion_model.pkl'
        
        # Try to resolve relative path from voice_module directory
        if not os.path.isabs(model_path):
            # Get the voice_module directory (parent directory of this file's grandparent)
            current_dir = os.path.dirname(__file__)  # src/emotion/
            src_dir = os.path.dirname(current_dir)    # src/
            voice_module_dir = os.path.dirname(src_dir)  # voice_module/
            model_path = os.path.join(voice_module_dir, model_path)
        
        if os.path.exists(model_path):
            logger.info(f"Loading emotion model from: {model_path}")
            self.load_model(model_path)
        else:
            logger.warning(f"Model not found at {model_path}, using placeholder models")
